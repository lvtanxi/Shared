 ...
 */
final class SharedConverter implements Converter {

  private final Parser parser;

  public SharedConverter(Parser parser) {
    if (parser == null) {
  ...
  @Override public <T> T fromString(String value, DataInfo info) throws Exception {
    ...
    SharedUtils.checkNull("data info", info);
  ...
  private <T> T toSet(String json, Class<?> type) throws Exception {
    ...
    Set<T> set = parser.fromJson(json, new TypeReference<Set<T>>() {
    }.getType());
  ...
  private <K, V, T> T toMap(String json, Class<?> keyType, Class<?> valueType) throws Exception {
    ...
    Map<K, V> map = parser.fromJson(json, new TypeReference<Map<K, V>>() {
    }.getType());
