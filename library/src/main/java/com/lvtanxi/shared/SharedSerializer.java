...
class SharedSerializer implements Serializer {
  ...
  SharedSerializer(LogInterceptor logInterceptor) {
    this.logInterceptor = logInterceptor;
  }

  @Override public <T> String serialize(String cipherText, T originalGivenValue) {
    SharedUtils.checkNullOrEmpty("Cipher text", cipherText);
    SharedUtils.checkNull("Value", originalGivenValue);
  ...
  @Override public DataInfo deserialize(String serializedText) {
    ...
    if (firstElement != null && firstElement.length() != 0) {
      try {
        ...
        logInterceptor.onLog("SharedSerializer -> " + e.getMessage());
      }
    }
